    ...
    <main className="flex min-h-screen flex-col items-center justify-between py-14 px-20">
      <div className="sm:absolute sm:left-1/2 sm:top-1/2 sm:-translate-x-1/2 sm:-translate-y-1/2 font-mono effect px-10 py-16 border-none rounded-lg flex flex-col md:flex-row items-center gap-20">
        <form className='items-center gap-6 flex-col flex'>
          ...
          <input type="text" value={userAdd} onChange={handleAddChange} className="px-2 py-1 rounded text-black" placeholder='Enter your address' />
          <button type="button" onClick={authUser} className='rounded w-2xl bg-green-700 px-6 py-2'>Proceed</button>
        </form>
        <span className='text-2xl'>OR</span>
        <form className='items-center gap-6 flex-col flex'>
          <div className='text-3xl text-center'>Create New</div>
          {newAdd && <div className='effect flex items-center px-4 py-1 gap-2 rounded justify-between'><div>{newAdd?.slice(0, 4)}...{newAdd?.slice(-4)}</div>
            <button
              type="button"
              onClick={(e) => {
          ...
          <button onClick={createNewUser} type="button" className='rounded w-2xl bg-green-700 px-6 py-2'>Proceed</button>
        </form>
      </div>
    </main>
  )
